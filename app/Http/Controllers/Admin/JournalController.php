...
class JournalController extends Controller
{
    ...
    {
        $title = "Journal List";
        $home_heading = HomeHeading::type('journal')->first();
        $type = 'Journal';
        $blogList = Journal::get();
        return view('Admin.journal.list', compact('blogList', 'title', 'type', 'home_heading'));
    }
...
//        $blog->banner_sub_title = $request->banner_sub_title ?? '';
       ...
       $other_meta_tag =   $blog->other_meta_tag = ($request->other_meta_tag) ? $request->other_meta_tag : '';
        ...
        if ($blog->save()) {
            session()->flash('success', 'Journal "' . $request->title . '" has been added successfully');
            return redirect(Helper::sitePrefix() . 'journal/');
        } else {
            return back()->with('message', 'Error while creating journal');
        }
    }
...
//        $blog->banner_sub_title = $request->banner_sub_title ?? '';
        ...
        if ($blog->save()) {
            session()->flash('success', 'journal "' . $request->title . '" has been updated successfully');
            return redirect(Helper::sitePrefix() . 'journal/');
        } else {
